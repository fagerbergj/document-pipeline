...
func TestChatRepo(t *testing.T) {
	db := openTestDB(t)
	chatRepo := db.Chats()
	msgRepo := db.ChatMessages()
	ctx := context.Background()

	rag := model.RAGConfig{Enabled: true, MaxSources: 5}
	session, err := chatRepo.Create(ctx, "system prompt", rag)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}

	got, found, err := chatRepo.Get(ctx, session.ID)
	if err != nil || !found {
	...
	newTitle := "Chat 1"
	updated, err := chatRepo.Update(ctx, session.ID, port.ChatSessionUpdates{Title: &newTitle})
	if err != nil || updated.Title != "Chat 1" {
		t.Errorf("Update: title=%q err=%v", updated.Title, err)
	}

	msgRepo.Append(ctx, session.ID, "user", "hello", nil)
	msgRepo.Append(ctx, session.ID, "assistant", "hi", []model.SourceRef{{DocumentID: "doc-1", Title: "Doc", Score: 0.9}})

	msgs, err := msgRepo.List(ctx, session.ID)
	if err != nil || len(msgs) != 2 {
		t.Errorf("List messages: len=%d err=%v", len(msgs), err)
	}
	if len(msgs[1].Sources) != 1 {
		t.Errorf("assistant sources: got %d", len(msgs[1].Sources))
	}

	chats, err := chatRepo.List(ctx, 10, nil)
	if err != nil || len(chats) != 1 {
		t.Errorf("List chats: len=%d err=%v", len(chats), err)
	}

	deleted, err := chatRepo.Delete(ctx, session.ID)
	if err != nil || !deleted {
		t.Errorf("Delete: deleted=%v err=%v", deleted, err)
	}
}

func TestChatRepo_ListPagination(t *testing.T) {
	chatRepo := openTestDB(t).Chats()
	ctx := context.Background()
	rag := model.RAGConfig{Enabled: true, MaxSources: 5}

	for i := 0; i < 5; i++ {
		chatRepo.Create(ctx, "", rag)
		time.Sleep(time.Millisecond)
	}

	page1, _ := chatRepo.List(ctx, 3, nil)
	if len(page1) != 3 {
		t.Fatalf("page1 len: %d", len(page1))
	}
	lastID := page1[len(page1)-1].ID
	page2, _ := chatRepo.List(ctx, 3, &lastID)
	if len(page2) != 2 {
